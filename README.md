...
- Creates a sensor in the August device with a count of the access codes set on the current lock and a binary sensor to monitor status of access code update requests.
- Exposes services to create, modify, and delete access codes.

## Requirements

- Home Assistant ≥ 2026.8.0
- A secure and publicly available Home Assistant either by your own means or via Home Assistant Cloud (Nabu Casa) as this integration requires "Cloud Push" update leveraging webhooks.  If not publicly available, the integration will fall back to "Cloud Polling."
...
>
> 1. The base [August Access](https://www.home-assistant.io/integrations/august/) or [Yale Access Bluetooth](https://www.home-assistant.io/integrations/yalexs_ble) Integration is already configured.
>
>    - The integration should work with both base integrations configured as well but the entities will be nested under the August Access device and not the Yale Access Bluetooth device.
...
Binary sensor to track the status of setting an access code. `on` == `Updating` and `off` == `Set`.

#### Sensor

Sensor with a count of the number of access codes active on the configured devices.  The sensor will be added to the August device.
